module variable_mod
    real(kind=4), public, allocatable :: psl(:,:), ts(:,:)
    real(kind=4), public, allocatable :: flds(:,:), fsds(:,:)
    real(kind=4), public, allocatable :: psl_l(:), ts_l(:)
    real(kind=4), public, allocatable :: flds_l(:), fsds_l(:)
    real(kind=4), public, allocatable :: pslm(:), tsm(:)
...
contains
    ...
    subroutine read_input_variables
        ...
        integer :: pslid, tsid, fldsid, fsdsid, maskid
        integer :: i, j
        ...
        if (masterproc) then
            ...
            ret = nf_inq_varid (ncid_input, "TS", tsid)
