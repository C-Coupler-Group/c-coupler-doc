...
path=${CODEROOT}/cesm/cesm1_0_5/scripts
CASENAME=$(basename "$CASEROOT")
pushd $path >& /dev/null
echo -e "./create_newcase -case $CASEROOT/config/cesm/cesm/$CASENAME -res $CESM_RES -compset $CESM_COMPSET -mach CCPL -max_tasks_per_node 12\n\n\n" > "${CASEROOT}/config/common/CESM_case_creation.log"
./create_newcase -case $CASEROOT/config/cesm/cesm/$CASENAME -res $CESM_RES -compset $CESM_COMPSET -mach CCPL -scratchroot /data/home/zhangcheng/CAM5 -din_loc_root_csmdata /data/home/zhangcheng/cesm1_0_3_hist_input -max_tasks_per_node 12 >> "${CASEROOT}/config/common/CESM_case_creation.log"
mv $CASEROOT/config/cesm/cesm/$CASENAME $CASEROOT/config/cesm/cesm/cesm_case_scripts 
report_logs "NOTICE" "The experiment model \"$COMPSET\" for this new simulation consists of the CESM model version CESM1_0_5. The log information for creating the corresponding CESM case is recorded in file ${CASEROOT}/config/common/CESM_case_creation.log"
popd > /dev/null 2>&1
